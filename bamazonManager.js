...
const bamazonObj = {
	...
	addInventory() {
		...
		prompt.get(properties, (err, res) => {
			...
			checkingProcesses.checkInputsAndUpdate(res);
		});
	},
	...
	addNewProduct() {
		...
		prompt.get(properties, (err, res) => {
			...
			checkingProcesses.checkProductInput(res);
		});
	}
...
const checkingProcesses = {
	...
	queryCheck(selectedId, queryAmount){
		...
		connection.query("SELECT item_id, stock_quantity FROM products WHERE item_id=?", [selectedId], (err, res) => {
			if (res.length === 0) {
				...
				updateFunctions.updateBamazonInventory(selectedId, queryAmount);
			};
		});
	},
	...
	checkProductInput(res) {
		...
		if (res.proName === '' && res.depName === '' && res.price === '' && res.stock === '') {
			...
			bamazonObj.addNewProduct();
		} else {
			// If everything else checks out, add the product to the database
			updateFunctions.addNewProToDatabase(res.proName, res.depName, res.price, res.stock);
		}
	}
};

const updateFunctions = {
	updateBamazonInventory(item_id, stock_quantity) {
		...
		connection.query("SELECT item_id, stock_quantity FROM products WHERE item_id=?", [managerId], (err, res) => {
			...
			if (err) console.log(err);
			this.updateDatabaseQuantities(databaseStock, managerQuantity, managerId);
		});
	},
	...
	updateDatabaseQuantities(databaseStock, managerQuantity, managerId) {
		...
		connection.query("UPDATE products SET stock_quantity=? WHERE item_id=?", [(databaseStock + managerQuantity), managerId], (err, res) => {
			...
			bamazonObj.viewProducts();
		});
	},

	addNewProToDatabase(proName, depName, price, stock) {
